...
# lr_list = [0.001, 0.002, 0.003, 0.005, 0.007, 0.01, 0.015, 0.02]  # 学习率
...
if dataset_name == 'Alice':
    ...
    # sub_train_size = train_size // 2
    # # 使用random_split随机选择部分数据
    # _, train_dataset = random_split(train_dataset, [sub_train_size, train_size - sub_train_size])
else:
...
for opt in range(len(opt_list)):
    ...
    for lr_index in range(len(lr_list)):
        ...
        for seed_index in range(len(seed_list)):
            ...
            m = {}  # 记录动量项
            h = {}  # 记录自适应步长项
            lin_m = {}  # 记录修正后的动量项  或是记录Adan中的vk
            lin_h = {}  # 记录修正后的自适应步长项  或是记录Adan中过去的梯度
            ...
            for epoch in range(Epoch):
                ...
                for i, (x, y) in enumerate(train_loader):
                    ...
                    if optimizer == 'Adam':
                        ...
                        for name, param in model.named_parameters():
                            ...
                            # 更新动量项和自适应步长项并记录梯度
                            m[name] = (1 - beta1) * param.grad + beta1 * m[name]
                            lin_m[name] = (1 - beta2) * (param.grad - lin_h[name]) + beta2 * lin_m[name]
                            h[name] = (1-beta3) * ((param.grad + beta2*(param.grad-lin_h[name])) ** 2) + beta3*h[name]
                            lin_h[name] = param.grad
                            # 更新模型参数
                            param.data = (param.data - gx_lr * (m[name]+beta2*lin_m[name]) / (eps+(h[name]**0.5))) / (1 + gx_lr * wd)
                ...
                with torch.no_grad():
                    ...
                    for j, (x, y) in enumerate(train_loader):
                        ...
                        output = model(x)
                        loss = criterion(output.view(-1, vocab_size), y.view(-1))  # 求损失
                        # perplexity = torch.exp(loss)  # 求困惑度
                        total_loss += loss
